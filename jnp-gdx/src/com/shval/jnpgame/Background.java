...
public class Background {
	...
	class BackgroundSprite extends BackgroundLayer {
		...
		void render(SpriteBatch batch) {
			...
			*/
			...
			//Gdx.app.debug(TAG, "Background sprite at " + gX + ", " + gY);
			sprite.draw(batch);
			//batch.draw(texture, gX, gY, gWidth , gHeight);
		}
		...
		void setResolution(int screenWidth, int screenHeight) {
			...
			sprite.setScale( (float) (width * screenWidth) / (float) (sprite.getRegionWidth() * 256),
							 (float) (height * screenHeight) / (float) (sprite.getRegionHeight() * 256));
	...
	void addSprite(Texture texture, float x, float y, int textureWidth, int textureHeight,
		...
		bgSprite.sprite = new Sprite(texture, textureWidth, textureHeight);
		bgSprite.x = x;
		bgSprite.y = y;
		bgSprite.vX = vX;
		bgSprite.vY = vY;
